...
| Audio/photo | Agent's authorized media tool or transcript/export | Preserve original locally, label generated transcription/description; no binary ingestion in CLI v0.2 |
...
Revoking access stops future reads. Source removal does not silently delete preserved material. Use a separate private state home when maintaining multiple accounts. Reconnecting to a different account invalidates previous upload approvals; review each destination explicitly.
